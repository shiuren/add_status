...
class FolderModel(models.TransientModel):
    ...
    folder_ids = fields.Many2many('folder.model', string="Dossier", compute="_compute_folder_line")

    @api.depends('date_from','date_end','partner_id')
    def _compute_folder_line(self):
        for rec in self:
            folder_ids = False
    ...
    def do_report_folder(self):
        return self.env.ref('mtx_folder.print_folder_report_complete_pdf').report_action(self)
    
    def get_report_folder_complete(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': 'Rapport Complete',
            'view_mode': 'list,form',
