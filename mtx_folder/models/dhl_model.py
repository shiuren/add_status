...
class DhlModel(models.Model):
   _name = 'dhl.model'
   _description = 'Dhl Model'
   _rec_name = "dhl_number"
   ...
   dhl_number = fields.Char(string="Numéro du Dossier DHL", required=True)
   ...
   doc_line_ids = fields.One2many('doc.dhl.line','doc_dhl_line_id',  string="Déclarants")
   attachement_line_ids = fields.One2many('attachement.dhl.lines','attachement_dhl_line_id',  string="Autres Pièces Jointes")
   ...
   def unlink(self):
        ...
        return super(DhlModel, self).unlink()
...
class AttachementDhlLines(models.Model):
   _name = 'attachement.dhl.lines'
   ...
   attachement_dhl_line_id = fields.Many2one('dhl.model')
...
class DocumentDhlLinei(models.Model):
    _name = 'doc.dhl.line'
    _description = 'Ligne de document'

    doc = fields.Binary(string='Document')
    doc_datas = fields.Binary(string='Database Data', attachment=False)
    doc_dhl_line_id = fields.Many2one('dhl.model')
