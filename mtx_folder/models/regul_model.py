...
class RegulModel(models.Model):
   _name = 'regul.model'
   _description = 'Regul Model'
   _rec_name = "regul_number"
   ...
   regul_number = fields.Char(string="Numéro du Regul", required=True)
   ...
   doc_line_ids = fields.One2many('doc.regul.line','doc_regul_line_id',  string="Déclarants")
   attachement_line_ids = fields.One2many('attachement.regul.lines','attachement_regul_line_id',  string="Autres Pièces Jointes")
   ...
   def unlink(self):
        ...
        return super(RegulModel, self).unlink()
...
class AttachementRegulLines(models.Model):
   _name = 'attachement.regul.lines'
   ...
   attachement_regul_line_id = fields.Many2one('regul.model')
...
class DocumentRegulLinei(models.Model):
    _name = 'doc.regul.line'
    _description = 'Ligne de document'

    doc = fields.Binary(string='Document')
    doc_datas = fields.Binary(string='Database Data', attachment=False)
    doc_regul_line_id = fields.Many2one('regul.model')
